...
namespace Quasar.Communication.Packets.Incoming.Marketplace
{
    class BuyOfferEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                Session.SendNotification("Oeps! Dit item is niet langer in de verkoop.");
                this.ReloadOffers(Session);
                return;
            }
            ...
            {
                Session.SendNotification("Oeps! Dit meubi is niet langer in de verkoop.");
                this.ReloadOffers(Session);
                return;
            }
            else
            {
                ...
                {
                    Session.SendNotification("Oeps! We bieden dit item niet meer aan.");
                    return;
                }

                if (Convert.ToInt32(Row["total_price"]) > Session.GetHabbo().Duckets)
                {
                    Session.SendNotification("Oeps! Je hebt niet voldoende Duckets om dit item te kunnen kopen.");
                    return;
                }
