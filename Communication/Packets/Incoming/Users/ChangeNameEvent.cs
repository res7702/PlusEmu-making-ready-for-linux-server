...
namespace Quasar.Communication.Packets.Incoming.Users
{
    class ChangeNameEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                Session.SendNotification("Oeps! Momenteel is het niet mogelijk om je naam te veranderen.");
                return;
            }
            ...
            {
                if (!QuasarEnvironment.GetGame().GetClientManager().UpdateClientUsername(Session, OldName, NewName))
                {
                    Session.SendNotification("Oeps! Je door jou gekozen nieuwe gebruikersnaam is ongeldig.");
                    return;
                }
                ...
                using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.SetQuery("INSERT INTO `logs_client_namechange` (`user_id`,`new_name`,`old_name`,`timestamp`) VALUES ('" + Session.GetHabbo().Id + "', @name, '" + OldName + "', '" + QuasarEnvironment.GetUnixTimestamp() + "')");
                    dbClient.AddParameter("name", NewName);
                    dbClient.RunQuery();
                }
                ...
                QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_Name", 1);
        ...
        private static bool CanChangeName(Habbo Habbo)
        {
            ...
            if (Habbo.Rank == 1 && Habbo.VIPRank == 0 && Habbo.LastNameChange == 0)
                return true;
            else if (Habbo.Rank == 1 && Habbo.VIPRank == 1 && (Habbo.LastNameChange == 0 || (QuasarEnvironment.GetUnixTimestamp() + 604800) > Habbo.LastNameChange))
                return true;
