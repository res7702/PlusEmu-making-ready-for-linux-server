...
namespace Quasar.Communication.Packets.Incoming.Rooms.Furni
{
    class OpenGiftEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Data == null)
            {
                Session.SendNotification("Oeps! Er is een fout opgetreden bij het openenen van dit cadeau.");
            ...
            else
            {
                ...
                {
                    Session.SendNotification("Oeps! Er is een fout opgetreden bij het openen van deze Rare-o-Matic box.");
                    Room.GetRoomItemHandler().RemoveFurniture(null, Present.Id, false);
            ...
            UserCache Purchaser = QuasarEnvironment.GetGame().GetCacheManager().GenerateUser(PurchaserId);
            if (Purchaser == null)
            {
                Session.SendNotification("Oeps! Er is een fout opgetreden bij het openenen van dit cadeau.");
                Room.GetRoomItemHandler().RemoveFurniture(null, Present.Id, false);
            ...
            if (!QuasarEnvironment.GetGame().GetItemManager().GetItem(Convert.ToInt32(Data["base_id"]), out BaseItem))
            {
                Session.SendNotification("Oeps! Dit cadeau bestaat niet meer.");
                Room.GetRoomItemHandler().RemoveFurniture(null, Present.Id, false);
