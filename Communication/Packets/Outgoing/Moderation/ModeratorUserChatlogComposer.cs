...
namespace Quasar.Communication.Packets.Outgoing.Moderation
{
    class ModeratorUserChatlogComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.ModeratorUserChatlogMessageComposer)
        {
           ...
           base.WriteString(QuasarEnvironment.GetGame().GetClientManager().GetNameById(UserId));
            using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (Visits != null)
                {
                    ...
                    foreach (DataRow Visit in Visits.Rows)
                    {
                        ...
                        Room Room = QuasarEnvironment.GetGame().GetRoomManager().LoadRoom(Convert.ToInt32(Visit["room_id"]));
                        ...
                        {
                            Visit["exit_timestamp"] = QuasarEnvironment.GetUnixTimestamp();
                        }
                        ...
                        if (Chatlogs != null)
                        {
                            ...
                            {
                                UserCache Habbo = QuasarEnvironment.GetGame().GetCacheManager().GenerateUser(Convert.ToInt32(Log["user_id"]));
