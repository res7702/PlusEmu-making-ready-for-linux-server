...
using Quasar.Communication.Packets.Outgoing.Rooms.Freeze;
using Quasar.HabboHotel.Rooms.Games.Teams;
using Quasar.HabboHotel.Items.Wired;


namespace Quasar.HabboHotel.Rooms.Games.Freeze
{
    public class Freeze
    {
        private Room _room;
        ...
        {
            return ExitTeleports.Values.ToList()[QuasarEnvironment.GetRandomNumber(0, ExitTeleports.Count - 1)];
        }
        ...
        private void CountTeamPoints()
        {
            ...
            foreach (RoomUser User in this._room.GetRoomUserManager().GetUserList().ToList())
            {
                ...
                if (User.FreezeLives == -1)
                    QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezeUser", 1);
            }
