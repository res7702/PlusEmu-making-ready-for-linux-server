...
namespace Quasar.HabboHotel.Rooms
{
    public class Gamemap
    {
        ...
        public Gamemap(Room room)
        {
            ...
            mStaticModel = QuasarEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            if (mStaticModel == null)
            {
                QuasarEnvironment.GetGame().GetRoomManager().LoadModel(room.ModelName);
                mStaticModel = QuasarEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            }
        ...
        private bool ConstructMapForItem(Item Item, Point Coord)
        {
            try
            {
                ...
                if (mItemHeightMap[Coord.X, Coord.Y] <= Item.TotalHeight)
                {
                    ...
                    else if (Item.GetBaseItem().InteractionType == InteractionType.GATE)// If this item is a gate, open, and on the floor, allow users to walk here.
                    {
                        if(Item.ExtraData == "1")
                        {
                            if (mGameMap[Coord.X, Coord.Y] != 3) // Ti
                                mGameMap[Coord.X, Coord.Y] = 1;
                        }
                        else
                        {
