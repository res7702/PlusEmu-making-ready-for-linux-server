...
namespace Quasar.HabboHotel.Rooms
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Quasar.HabboHotel.Rooms.RoomManager");
        ...
        public RoomManager()
        {
            ...
            Console.WriteLine(CurrentTime + "» Navigator " + this._roomModels.Count + " kamer models");
            Console.ForegroundColor = ConsoleColor.DarkGray;
            Console.WriteLine(CurrentTime + "» Navigator » Geladen!");
            Console.ResetColor();
        }
        ...
        public void OnCycle()
        {
                    ...
                    foreach (Room Room in this._rooms.Values.ToList())
                    {
                        ...
                        else
                        {
                            ...
                            if (Room.IsLagging >= 30)
                            {
                                Room.isCrashed = true;
                                UnloadRoom(Room);
                                Logging.WriteLine("[KamerMgr] Kamer gecrasht (Taak niet voltooid in 30 seconden): [" + Room.RoomId +"]");
                            }
                        }
                    }
                }
            ...
            {
                Logging.LogCriticalException("Fout met de Kamer Manager: " + e);
            }
        }
        ...
        public void Dispose()
        {
            ...
            foreach (Room Room in this._rooms.Values.ToList())
            {
                ...
                string CurrentTime = DateTime.Now.ToString("HH:mm:ss" + " | ");
                Console.WriteLine(CurrentTime + "» Afsluiten » Meubels » Opslaan " + String.Format("{0:0.##}", ((double)i / length) * 100) + "%");
                i++;
            }

            string CurrentTime1 = DateTime.Now.ToString("HH:mm:ss" + " | ");
            Console.WriteLine(CurrentTime1 + "» Afsluiten » Kamers » Opslaan");
