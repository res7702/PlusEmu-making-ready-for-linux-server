        ...
        public string GetUsername()
        {
            ...
            if (GetClient() != null)
            {
                if (GetClient().GetHabbo() != null)
                {
                    ...
                    return QuasarEnvironment.GetUsernameById(HabboId);
    ...
    public void OnChat(int Colour, string Message, bool Shout)
        {
            ...
            if (Shout)
                Packet = new ShoutComposer(VirtualId, ColouredMessage, QuasarEnvironment.GetGame().GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
            else
                Packet = new ChatComposer(VirtualId, ColouredMessage, QuasarEnvironment.GetGame().GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
        ...
        public GameClient GetClient()
        {
            ...
            if (mClient == null)
                mClient = QuasarEnvironment.GetGame().GetClientManager().GetClientByUserID(HabboId);
            return mClient;
        }
        ...
        {
            if (mRoom == null)
                if (QuasarEnvironment.GetGame().GetRoomManager().TryGetRoom(RoomId, out mRoom))
                    return mRoom;
