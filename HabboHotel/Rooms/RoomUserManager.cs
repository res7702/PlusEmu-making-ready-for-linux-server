...
namespace Quasar.HabboHotel.Rooms
{
    public class RoomUserManager
    {
        ...
        public bool AddAvatarToRoom(GameClient Session)
        {
            ...
            {
                Session.SendWhisper("Deze kamer staat te koop voor " + _room.SalePrice + " Duckets. Typ :koopkamr om de kamer over te kopen!");
            }
            else if (_room.ForSale && _room.GetRoomUserManager().GetRoomUserByHabbo(_room.OwnerName) == null)
            {
                foreach (RoomUser _User in _room.GetRoomUserManager().GetRoomUsers())
                {
                    ...
                    {
                        _User.GetClient().SendWhisper("Oeps! Deze kamer staat niet langer meer te koop.");
                    }
                }
                _room.ForSale = false;
                _room.SalePrice = 0;
            ...
            /*
            if (Session.GetHabbo().GetPermissions().HasRight("mod_tools") && !Session.GetHabbo().DisableForcedEffects && Session.GetHabbo().Rank >= 5)
        ...
        public void RemoveUserFromRoom(GameClient Session, Boolean NotifyClient, Boolean NotifyKick = false)
        {
            try
            {
                ...
                if (User != null)
                {
                    ...
                    {
                        dbClient.runFastQuery("UPDATE user_roomvisits SET exit_timestamp = '" + QuasarEnvironment.GetUnixTimestamp() + "' WHERE room_id = '" + _room.RoomId + "' AND user_id = '" + Session.GetHabbo().Id + "' ORDER BY exit_timestamp DESC LIMIT 1");
            ...
            if (Session.GetHabbo() != null && Session.GetHabbo().Id == _room.OwnerId && _room.ForSale)
            {
                ...
                foreach (RoomUser m in GetRoomUsers())
                {
                    ...
                    {
                        m.GetClient().SendWhisper("Oeps! Deze kamer is niet langer te koop.");
                    }
                }
            }
        }
        ...
        public void OnCycle()
        {
            ...
            try
            {
                ...
                foreach (RoomUser User in GetUserList().ToList())
                {
                    ...
                    // Geef Achievement -> Cycle naar 0 bij 21 !
                    if (User.DiceTotal == 21)
                    {
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoBeWinner", 1, false);
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 21, false);
                        {
                            User.DiceTotal = 0;
                        }
                    }

                    #region CasinoGetDiceScore Cases


                    // 22
                    if (User.DiceTotal == 22)
                    {
                        User.DiceTotal = 0;
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 22, false);
                    }

                    // 23
                    if (User.DiceTotal == 23)
                    {
                        User.DiceTotal = 0;
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 23, false);
                    }

                    // 24
                    if (User.DiceTotal == 24)
                    {
                        User.DiceTotal = 0;
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 24, false);
                    }

                    // 25
                    if (User.DiceTotal == 25)
                    {
                        User.DiceTotal = 0;
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 25, false);
                    }

                    // 26
                    if (User.DiceTotal == 26)
                    {
                        User.DiceTotal = 0;
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 26, false);
                    }

                    // 27
                    if (User.DiceTotal == 27)
                    {
                        User.DiceTotal = 0;
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 27, false);
                    }

                    // 28
                    if (User.DiceTotal == 28)
                    {
                        User.DiceTotal = 0;
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 28, false);
                    }

                    // 29
                    if (User.DiceTotal == 29)
                    {
                        User.DiceTotal = 0;
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 29, false);
                    }

                    // [Achievement ACH_CasinoRoyalFlush]
                    if (User.DiceTotal == 30)
                    {
                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoRoyalFlush", 1, false);
                        {
                            User.DiceTotal = 0;
                            QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_CasinoGetDiceScore", 30, false);
                        }
                    }
                    #endregion
                ...
                {
                    GameClient client = QuasarEnvironment.GetGame().GetClientManager().GetClientByUserID(toRemove.HabboId);
                    if (client != null)
                    {
            ...
            catch (Exception e)
            {
                ...
                Logging.LogCriticalException("Aangetaste Kamer [" + rId + "] - " + e.ToString());
            }
        }
        ...
        public void UpdateUserStatus(RoomUser User, bool cyclegameitems)
        {
            ...
            try
            {
                ...
                if (QuasarEnvironment.GetUnixTimestamp() > QuasarEnvironment.GetUnixTimestamp() + User.SignTime)
                {
                    if (User.Statusses.ContainsKey("sign"))
                    {
                ...
                foreach (Item Item in ItemsOnSquare.ToList())
                {
                    ...
                    switch (Item.GetBaseItem().InteractionType)
                    {
                        ...
                        case InteractionType.FX_PROVIDER:
                                ...
                                if (User?.GetClient()?.GetHabbo()?.Effects() == null || User.IsBot || Item?.Data == null || 
                                    (Item.Data.EffectId == 0 && !Item.Data.ItemName.Contains("remove")))
                                {
                                    return;
                                }

                                User.GetClient().GetHabbo().Effects().ApplyEffect(Item.Data.EffectId);
                                Item.UpdateState(false, true);
                        ...
                        case InteractionType.ARROW:
                            {
                                if (User.GoalX == Item.GetX && User.GoalY == Item.GetY)
                                {
                                    ...
                                    if (!QuasarEnvironment.GetGame().GetRoomManager().TryGetRoom(User.GetClient().GetHabbo().CurrentRoomId, out Room))
                                        return;
                        ...
                        case InteractionType.PINATA:
                            {
                                int HitsGiven = 0;
                                if(!User.IsWalking || User.UserId != Item.UserID || string.IsNullOrWhiteSpace(Item.ExtraData) || 
                                    !int.TryParse(Item.ExtraData, out HitsGiven) || HitsGiven > 0 || User.CurrentEffect != 158)
                                {
                                    break;
                                }
                                ...
                                Item.ExtraData = HitsGiven.ToString();
                                Item.UpdateState();

                                if (HitsGiven == 1)
                                {
                                    QuasarEnvironment.GetGame().GetPinataManager().ReceiveCrackableReward(User, _room, Item);
                                }

                                 QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataBreaker", 1);
