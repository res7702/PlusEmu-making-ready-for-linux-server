...
namespace Quasar.HabboHotel.Navigator
{
        ...
        public static void Search(ServerPacket Message, SearchResultList SearchResult, string SearchData, GameClient Session, int FetchLimit)
        {
            ...
            switch (SearchResult.CategoryType)
            {
                ...
                case NavigatorCategoryType.QUERY:
                    {
                        ...
                        if (SearchData.ToLower().StartsWith("owner:"))
                        {
                            if (SearchData.Length > 0)
                            {
                                ...
                                if (GetRooms != null)
                                {
                                    ...
                                    {
                                        RoomData RoomData = QuasarEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row);
                        ...
                        else if (SearchData.ToLower().StartsWith("tag:"))
                        {
                            ...
                            ICollection<RoomData> TagMatches = QuasarEnvironment.GetGame().GetRoomManager().SearchTaggedRooms(SearchData);
                        ...
                        else if (SearchData.ToLower().StartsWith("group:"))
                        {
                            ...
                            ICollection<RoomData> GroupRooms = QuasarEnvironment.GetGame().GetRoomManager().SearchGroupRooms(SearchData);
                            ...
                            if (SearchData.Length > 0)
                            {
                                ...
                                if (Table != null)
                                {
                                    foreach (DataRow Row in Table.Rows)
                                    {
                                        ...
                                        RoomData RData = QuasarEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row);
                ...
                case NavigatorCategoryType.FEATURED:
                    ...
                    ICollection<FeaturedRoom> Featured = QuasarEnvironment.GetGame().GetNavigator().GetFeaturedRooms();
                    foreach (FeaturedRoom FeaturedItem in Featured.ToList())
                    {
                        if (FeaturedItem == null)
                            continue;

                        RoomData Data = QuasarEnvironment.GetGame().GetRoomManager().GenerateRoomData(FeaturedItem.RoomId);
                        if (Data == null)
                            continue;
                ...
                case NavigatorCategoryType.POPULAR:
                    {
                        List<RoomData> PopularRooms = QuasarEnvironment.GetGame().GetRoomManager().GetPopularRooms(-1, FetchLimit);
                ...
                case NavigatorCategoryType.RECOMMENDED:
                    {
                        List<RoomData> RecommendedRooms = QuasarEnvironment.GetGame().GetRoomManager().GetRecommendedRooms(FetchLimit);
                ...
                case NavigatorCategoryType.CATEGORY:
                    {
                        List<RoomData> GetRoomsByCategory = QuasarEnvironment.GetGame().GetRoomManager().GetRoomsByCategory(SearchResult.Id, FetchLimit);
                ...
                case NavigatorCategoryType.MY_FAVORITES:
                    ...
                    {
                        RoomData Room = QuasarEnvironment.GetGame().GetRoomManager().GenerateRoomData(Id);
                        if (Room == null)
                            continue;
                ...
                case NavigatorCategoryType.MY_GROUPS:
                    ...
                    foreach (Group Group in QuasarEnvironment.GetGame().GetGroupManager().GetGroupsForUser(Session.GetHabbo().Id).ToList())
                    {
                        if (Group == null)
                            continue;

                        RoomData Data = QuasarEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId);
                        if (Data == null)
                            continue;
                ...
                case NavigatorCategoryType.MY_RIGHTS:
                    ...
                    using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        {
                            RoomData Data = QuasarEnvironment.GetGame().GetRoomManager().GenerateRoomData(Convert.ToInt32(Row["room_id"]));
                            if (Data == null)
                                continue;
                ...
                case NavigatorCategoryType.TOP_PROMOTIONS:
                    {
                        List<RoomData> GetPopularPromotions = QuasarEnvironment.GetGame().GetRoomManager().GetOnGoingRoomPromotions(16, FetchLimit);
                ...
                case NavigatorCategoryType.PROMOTION_CATEGORY:
                    {
                        List<RoomData> GetPromotedRooms = QuasarEnvironment.GetGame().GetRoomManager().GetPromotedRooms(SearchResult.Id, FetchLimit);
