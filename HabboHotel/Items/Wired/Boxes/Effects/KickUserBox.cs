    ...
    class KickUserBox : IWiredItem, IWiredCycle
    {
        ...
        public bool Execute(params object[] Params)
        {
            ...
            if (!this._toKick.Contains(Player))
            {
                ...
                if (Player.Rank >= 2 || this.Instance.OwnerId == Player.Id)
                {
                    Player.GetClient().SendMessage(new WhisperComposer(User.VirtualId, "Oeps! Deze gebruiker kan niet worden gekickt. (Personeel).", 0, 0));
                    return false;
                }
