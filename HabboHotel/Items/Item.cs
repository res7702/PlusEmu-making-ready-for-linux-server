...
using Quasar.HabboHotel.Items.Interactor;
...
namespace Quasar.HabboHotel.Items
{
    ...
    public class Item
    {
        ...
        {
            ItemData Data = null;
            if (QuasarEnvironment.GetGame().GetItemManager().GetItem(BaseItem, out Data))
            {
                this.Id = Id;
                this.RoomId = RoomId;
                this._room = Room;
                ...
                this.UserID = Userid;
                this.Username = QuasarEnvironment.GetUsernameById(Userid);
                ...
                this.Interactor = Item.GetInteractor(this);
            }
        }
        ...
        public static IFurniInteractor GetInteractor(Item item)
        {
            bool IsWired = item.IsWired;
            if (IsWired)
            {
                return new InteractorWired();
            }
            // Wired Achievement - Reacties
            bool IsTrigger = item.IsTrigger;
            if (IsTrigger)
            {
                return new InteractorWired();
            }
            // Wired Achievement - Acties
            bool IsAction = item.IsAction;
            if (IsAction)
            {
                return new InteractorWired();
            }
            // Wired Achievement - Voorwaarden
            bool IsCondition = item.IsCondition;
            if (IsCondition)
            {
                return new InteractorWired();
            }
            // Wired Achievement - Highscores
            bool IsHighscore = item.IsHighscore;
            if (IsHighscore)
            {
                return new InteractorWired();
            }
            ...
            switch (item.GetBaseItem().InteractionType)
            {
                ...
                case InteractionType.MUSIC_DISC:
                    return new InteractorMusicDisc(item);
                ...
                if (UpdateCounter <= 0)
                {
                    ...
                    switch (GetBaseItem().InteractionType)
                    {
                        ...
                        case InteractionType.GATE_VIP:
                            ...
                            if (!_room.GetGameMap().SquareHasUsers(GetX, GetY))
                            {
                                ExtraData = "0";
                                UpdateState();
                                break;
                        ...
                        case InteractionType.POKER:
                            {
                                ...
                                if (ExtraData == "-1")
                                {
                                    var num = RandomNumber.GenerateRandom(1, 52);
                                    ExtraData = num.ToString();
                                }
        ...
        public ItemData GetBaseItem()
        {
            ...
            {
                ItemData I = null;
                if (QuasarEnvironment.GetGame().GetItemManager().GetItem(this.BaseItem, out I))
                    this._data = I;
            }
        ...
        public void UserWalksOnFurni(RoomUser user)
        {
            ...
            if (GetBaseItem().InteractionType == InteractionType.PRESSURE_PAD)
                QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(user.GetClient(), "ACH_WiredExpertUsePressureTile", 1);
        }
