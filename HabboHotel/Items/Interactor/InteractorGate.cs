...
namespace Quasar.HabboHotel.Items.Interactor
{
    internal class InteractorGate : IFurniInteractor
    {
        ...
        {
            if (!hasRights) return;
            if (item == null || item.GetBaseItem() == null || item.GetBaseItem().InteractionType != InteractionType.GATE) return;


            var modes = item.GetBaseItem().Modes - 1;
            if (modes <= 0) item.UpdateState(false, true);

            if (item.GetRoom() == null || item.GetRoom().GetGameMap() == null || item.GetRoom().GetGameMap().SquareHasUsers(item.GetX, item.GetY)) return;
            ...
            if (newMode == 0 && !item.GetRoom().GetGameMap().itemCanBePlacedHere(item.GetX, item.GetY)) return;

            item.ExtraData = newMode.ToString();
            item.UpdateState();
            item.GetRoom().GetGameMap().updateMapForItem(item);
            item.GetRoom().GetWired().TriggerEvent(WiredBoxType.TriggerStateChanges, item.GetRoom().GetRoomUserManager().GetRoomUserByHabbo(session.GetHabbo().Id), item);
        }
