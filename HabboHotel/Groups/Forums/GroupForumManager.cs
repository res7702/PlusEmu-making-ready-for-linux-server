...
namespace Quasar.HabboHotel.Groups.Forums
{
    public class GroupForumManager
    {
        ...
        public bool TryGetForum(int Id, out GroupForum Forum)
        {
            ...
            Group Gp;
            if (!QuasarEnvironment.GetGame().GetGroupManager().TryGetGroup(Id, out Gp))
                return false;
        ...
        {
            GroupForum F;
            return QuasarEnvironment.GetGame().GetGroupManager().GetGroupsForUser(Userid).Where(c => TryGetForum(c.Id, out F)).Select(c => GetForum(c.Id)).ToList();
        }
