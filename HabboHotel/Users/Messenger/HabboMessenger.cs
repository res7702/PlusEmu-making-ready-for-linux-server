...
namespace Quasar.HabboHotel.Users.Messenger
{
    public class HabboMessenger
    {
        ...
        public void ProcessOfflineMessages()
        {
            ...
            using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetMessages != null)
                {
                    GameClient Client = QuasarEnvironment.GetGame().GetClientManager().GetClientByUserID(this._userId);
                    if (Client == null)
                        return;
        ...
        public void Destroy()
        {
            IEnumerable<GameClient> onlineUsers = QuasarEnvironment.GetGame().GetClientManager().GetClientsById(_friends.Keys);
        ...
        public void CreateFriendship(int friendID)
        {
            ...
            GameClient User = QuasarEnvironment.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (User != null)
                QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User, "ACH_FriendListSize", 1);

            GameClient thisUser = QuasarEnvironment.GetGame().GetClientManager().GetClientByUserID(_userId);
            if (thisUser != null)
                QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(thisUser, "ACH_FriendListSize", 1);
        }
        ...
        public void OnNewRequest(int friendID)
        {
            if (!_requests.ContainsKey(friendID))
                _requests.Add(friendID, new MessengerRequest(_userId, friendID, QuasarEnvironment.GetGame().GetClientManager().GetNameById(friendID)));
        }
        ...
        public void SendInstantMessage(int ToId, string Message)
        {
            ...
            if (GetClient().GetHabbo().MessengerSpamCount >= 12)
            {
                ...
                GetClient().SendNotification("Hyena! Je moet iets rustiger aan proberen te chatten.\n\nJe kan zometeen weer een bericht versturen!");
                return;
            }
            else if (GetClient().GetHabbo().MessengerSpamTime > QuasarEnvironment.GetUnixTimestamp())
            {
                ...
                GetClient().SendNotification("Hyena! moet iets rustiger aan proberen te chatten.\n\nJe kunt weer chatten in " + Time + " seconden.");
                return;
            }
            ...
            GameClient Client = QuasarEnvironment.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (Client == null || Client.GetHabbo() == null || Client.GetHabbo().GetMessenger() == null)
            {
                using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
                {
        ...
        {
            IEnumerable<GameClient> MyFriends = QuasarEnvironment.GetGame().GetClientManager().GetClientsById(this._friends.Keys);
        ...
        {
            return QuasarEnvironment.GetGame().GetClientManager().GetClientByUserID(this._userId);
        }
