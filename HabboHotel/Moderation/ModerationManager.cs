...
namespace Quasar.HabboHotel.Moderation
{
    ...
    {
        private static ILog log = LogManager.GetLogger("Quasar.HabboHotel.Moderation.ModerationManager");
        ...
        public ModerationManager()
        {
            ...
            Console.WriteLine(CurrentTime + "» Moderator » Geladen!");
            Console.ResetColor();
        ...
        public void Init()
        {
            ...
            Console.WriteLine(CurrentTime + "» " + "Moderator " + (this._userPresets.Count + this._roomPresets.Count) + " templates");
            Console.WriteLine(CurrentTime + "» " + "Moderator " + this._userActionPresetCategories.Count + " categorieën");
            Console.WriteLine(CurrentTime + "» " + "Moderator " + this._userActionPresetMessages.Count + " acties");
            Console.WriteLine(CurrentTime + "» " + "Moderator " + this._bans.Count + " verbanningen");
          }
        ...
        public void BanUser(string Mod, ModerationBanType Type, string BanValue, string Reason, double ExpireTimestamp)
        {
            ...
            using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("REPLACE INTO `bans` (`bantype`, `value`, `reason`, `expire`, `added_by`,`added_date`) VALUES ('" + BanType + "', '" + BanValue + "', @reason, " + ExpireTimestamp + ", '" + Mod + "', '" + QuasarEnvironment.GetUnixTimestamp() + "');");
        ...
        public bool MachineBanCheck(string MachineId)
        {
            ...
            if (QuasarEnvironment.GetGame().GetModerationManager().IsBanned(MachineId, out MachineBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        QuasarEnvironment.GetGame().GetModerationManager().RemoveBan(MachineId);
                        return false;
                    }
                }
            }
        ...
        public bool UsernameBanCheck(string Username)
        {
            ...
            if (QuasarEnvironment.GetGame().GetModerationManager().IsBanned(Username, out UsernameBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        QuasarEnvironment.GetGame().GetModerationManager().RemoveBan(Username);
                        return false;
                    }
                }
            }
